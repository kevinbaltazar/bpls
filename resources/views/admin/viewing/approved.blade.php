<x-admin-layout>
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center mb-4">
      ...
      <form action="{{route('admin.search')}}" method="get">
        @csrf
        <div>
            <input type="hidden" name="hidden" value="approved">
            <label for="search" class="sr-only">search</label>
            <input type="text" name="search" id="search" class=" justify-between shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-56 sm:text-sm border-gray-300 rounded-md" placeholder="Search Control Number...">
        </div>
      </form>
    </div>
